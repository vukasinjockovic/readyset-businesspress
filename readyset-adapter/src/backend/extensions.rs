    ...
    Handler: 'static + QueryHandler,
{
    ...
    pub(super) async fn create_shallow_cache_core(
        ...
        // RSC ruling (upstream-sync 2026-08-10): shallow caches have no
        // dataflow deltas, so RSC must never treat them as reactive. Mark
        // every shallow creation suspect (both DDL and auto-creation paths
        // funnel through here) so the PHP side excludes them from twin/HYD
        // eligibility. Creation itself proceeds — shallow stays a valid
        // upstream feature, just outside RSC.
        if rsc_admission::admission_mode() != rsc_admission::AdmissionMode::Off {
            rsc_admission::mark_suspect(
                &name.display_unquoted().to_string(),
                rsc_admission::CLASS_SHALLOW_CACHE,
            );
        }
